...
swisstopo hand-off: it stores one generated GPX temporarily in R2 so the
swisstopo app can fetch it by public URL. Desktop users receive a locally
rendered QR code; small touch devices follow the same hand-off URL directly.
...
- GitHub Actions and GitHub Pages provide continuous validation and static
  deployment.
- An optional Cloudflare Worker plus R2 binding provides short-lived GPX URLs
  only for the user-requested swisstopo transfer.
...
Only an explicit swisstopo transfer action uploads the already-generated GPX.
The Worker validates a small GPX payload, stores it below an isolated R2 prefix
with an unguessable UUID and expiration metadata, and serves it publicly long
enough for swisstopo to fetch it. A scheduled cleanup removes expired objects.
...
| swisstopo hand-off | `src/share/`, `src/components/RouteExportDialog.tsx`, `workers/swisstopo-gpx-share/` | Builds the documented `/u/` URL, renders a desktop QR or follows it directly on small touch devices, uploads GPX only on explicit request, and expires temporary R2 objects |
...
- the serializer writes a GPX 1.1 track with metadata bounds;
...
- the browser base64url-encodes that URL behind the official
  ...
  while small coarse-pointer layouts follow the swisstopo link directly because
  a phone cannot conveniently scan a QR code displayed on its own screen;
...
| swisstopo temporary share | Keep the export dialog and local GPX download usable; report a localized share error without changing route state |
...
- **Current itineraries and metrics** — GPX parsing, projection, segmented
  read-only geometry, export, swisstopo URL/QR generation, distance and elevation processing, the global
...
`workers/swisstopo-gpx-share/`; its example Wrangler configuration binds R2,
restricts browser upload origins, defines the temporary lifetime, and schedules
