...
Desktop users receive a locally
...
document. That document may be generated by Via Helvetica or retained from the
current read-only GPX import. The Worker validates a small GPX payload, stores it
below an isolated R2 prefix with an unguessable UUID and expiration metadata,
and serves it publicly for 24 hours by default so swisstopo can fetch it. A
scheduled cleanup removes expired objects.
